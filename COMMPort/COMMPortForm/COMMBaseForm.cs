...
{
	public partial class COMMBaseForm : Form
	{
		...
		public COMMBaseForm()
		{
			...
			this._mouseMsgFilter = new AppMouseMessageHandler(this);
			...
			this._border3DStyle = System.Windows.Forms.Border3DStyle.RaisedInner;
		...
		private DialogResult ShowDialogInternal(Component controlOrItem, Point offset)
		{
			...
			if (this._isShowDialogAgain == true)
			{
				return System.Windows.Forms.DialogResult.None;
			}
		...
		{
			readonly COMMBaseForm _layerForm;

			/// <summary>
			/// 
			/// </summary>
			/// <param name="layerForm"></param>
			public AppMouseMessageHandler(COMMBaseForm layerForm)
			{
			...
			public bool PreFilterMessage(ref Message m)
			{
				...
				if((m.Msg == 0x201)&& this._layerForm.Visible && !NativeMethods.GetWindowRect(this._layerForm.Handle).Contains(MousePosition))
				{
					this._layerForm.Hide();//之所以不Close是考虑应该由调用者负责销毁
					_return = true;
				}
